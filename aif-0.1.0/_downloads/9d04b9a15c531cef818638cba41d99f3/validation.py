...
def _quantize_matrix(matrix: Gf.Matrix4d, tolerance: float = TRANSFORM_TOLERANCE) -> tuple[float, ...]:
    ...
    return tuple(
        round(float(matrix[i][j]) / tolerance) * tolerance for i in range(4) for j in range(4)
    )
...
def _mesh_has_static_topology(mesh: UsdGeom.Mesh) -> bool:
    ...
    return not any(
        attr.ValueMightBeTimeVarying() for attr in (points_attr, indices_attr, counts_attr)
    )
...
def _mesh_geometry_signature(mesh: UsdGeom.Mesh) -> tuple | None:
    ...
    point_sig = tuple(
        tuple(round(float(component), MESH_POINT_PRECISION) for component in point) for point in points
    )
...
@omni.asset_validator.register_rule("Geometry")
@omni.asset_validator.register_requirements(cap.GeometryRequirements.VG_008, override=True)
class CoincidentMeshChecker(omni.asset_validator.BaseRuleChecker):
    """
