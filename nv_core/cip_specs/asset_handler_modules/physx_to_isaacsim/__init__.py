...
def modify_stage(input_stage: Usd.Stage, output_stage: Usd.Stage):
    ...
    report = manager.run(
        input_stage=input_stage, profile=profile, package_root=transformed_package_root
    )
    ...
    if not transformed_package_root.is_dir():
        raise RuntimeError(
            f"Expected transformed asset directory not found: {transformed_package_root}"
        )

    output_root_layer = output_stage.GetRootLayer()
    if output_root_layer is None:
        raise RuntimeError(
            f"Failed to get output root layer"
        )
    ...
    if not transformed_root_layer_path.exists():
        raise RuntimeError(
            f"Transformed root layer not found at expected location: "
            f"{transformed_root_layer_path}"
        )
    ...
    if transformed_layer is None:
        raise RuntimeError(
            f"Failed to open transformed root layer: {transformed_root_layer_path}"
        )
