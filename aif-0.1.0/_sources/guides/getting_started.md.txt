...
| **Data Center / Facilities Engineer**                       | Validate that OpenUSD assets for CDUs, CRAHs, UPS units, and compute racks carry the correct thermal, electrical, and connection-point metadata for AI Factory digital twin simulations.                                                                    |
...
| **Data center infrastructure equipment** (CDU, CRAH, UPS, Compute Rack) | [AIF-Entity](../profiles/aif-entity.md) (single profile; set `aif:core:assetClass` to your equipment class) | Core metadata and connection points (capabilities) plus thermal/electrical (features), gated per equipment class. See the profile authoring guide for equipment-specific attributes and sublayer composition.               |
...
| **Author** a new asset from scratch                                     | Study the sample assets in `sample_content/common_assets/` for robotics structure. For data center equipment, see the [AIF Pipeline Samples documentation](https://nvidia-omniverse.github.io/aif-pipeline-samples/) and the [AIF-Entity profile authoring guide](../profiles/aif-entity.md). Then validate against your target profile. |
