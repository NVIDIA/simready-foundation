...
Requires **Python >=3.10,<3.13** (Python 3.12 recommended). Install the SimReady validation library from the repository's `requirements.txt`:
...
6. **Install:** `pip install -r nv_core/validator_sample/requirements.txt`
...
```{note}
For the full acceptance lifecycle (domain identification, gap analysis, prototyping, QA, delivery), see the [Acceptance Workflow](acceptance_workflow.md).
```
